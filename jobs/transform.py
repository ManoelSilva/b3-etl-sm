...
def remove_all_null_columns(df, keep_columns=None):
    ...
    non_all_null_cols = [c for c, count in non_null_counts.items() if count > 0 or c in keep_columns]
...
df = df.withColumnRenamed('cod', 'code').withColumnRenamed('asset', 'ticker').withColumnRenamed('date', 'reference_date')
...
if 'part' in df.columns:
    df = df.withColumn('part', F.regexp_replace('part', ',', '.').cast(DoubleType()))
else:
...
initial_date = df_dates.agg(F.min('reference_date')).collect()[0][0] if df_dates.count() > 0 else None
...
window_spec = Window.partitionBy('code').orderBy('reference_date').rowsBetween(Window.unboundedPreceding, Window.currentRow)
df = df.withColumn('part_sum_from_initial_date', F.sum(F.col('part')).over(window_spec))
...
if 'reference_date' in df.columns and 'code' in df.columns:
    df = df.filter(F.col('reference_date').isNotNull() & F.col('code').isNotNull())
    print("Output path:", output_path)
    print("Sample data before write:")
    df.show(5)
    # Check if DataFrame is not empty
    if not df.rdd.isEmpty():
        df.write.partitionBy('code', 'reference_date').mode('overwrite').parquet(output_path)
    else:
